...
CREATE TABLE `openapi_app` (
  ...
  `max_qps` int(11) DEFAULT NULL COMMENT '每秒限流阈值',
  `day_limit` int(11) DEFAULT NULL COMMENT '每日限额',
...
CREATE TABLE `openapi_order` (
  ...
  `out_trade_no` bigint(20) NOT NULL COMMENT '本地订单号',
  `user_id` bigint(20) NOT NULL COMMENT '本地用户ID',
  `amount` int(11) NOT NULL COMMENT '金额(分)',
  `status` tinyint(1) NOT NULL DEFAULT '0' COMMENT '0创建 1已支付 2失败',
